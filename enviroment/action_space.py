...
class ActionSpace(object):
    ...
    def getRandomOrderbookState(self):
        random_state, random_index = self.orderbook.getRandomState(max(self.T))
        self.set_volatility(random_index, random_state)
...
#             print(runtime)
    ...
    def update(self, t, i, force_execution=False, orderbookIndex = None ):
        ...
        if self.action == None:
            self.action = self.createAction(level=a, state=aiState, force_execution=force_execution,orderbookIndex=orderbookIndex)
            #print('Start ref',self.action.referencePrice)
        else:
            self.action = self.updateAction(action = self.action, level=a, state=aiState, force_execution=force_execution,orderbookIndex=orderbookIndex)
            #print('Updated ref',self.action.referencePrice)
        
        self.action, counterTrades = self.action.run(self.orderbook)
        i_next = self.determineNextInventory(self.action)
        t_next = self.determineNextTime(t)
        
        if i_next == 0:
            reward = self.action.getReward()
            #print('Final reward',reward)
        else:
            reward, volumeRatio = self.action.calculateRewardWeighted(counterTrades, self.I[-1])
            #print('Weighted reward',reward)
        state_next = ActionState(self.action.getState().getT(), self.action.getState().getI(), self.action.getState().getMarket())
        state_next.setT(t_next)
        state_next.setI(i_next)
        
        self.ai.learn(
            state1=self.action.getState(),
            action1=self.action.getA(),
            reward=reward,
            state2=state_next
        )
        return (t_next, i_next, self.action.orderbookIndex), reward, a
    ...
    def train(self, episodes=1, force_execution=False):
        ...
        for episode in range(int(episodes)):
            if episode%100 == 0 :
                ...
                np.save('./q_models/q_{}.npy'.format(episode), self.ai.q)
            rewards = []
            actions = []
            for t in self.T:
                logging.info("\n"+"t=="+str(t))
