...
class GymWrapper(py_environment.PyEnvironment):
  ...
  def _reset(self):
    ...
    self._info = self._gym_env.volatility
    self._done = False
    ...
    if self._morl:
      return ts.restart_morl(observation, reward_size = self._reward_size) 
    else:
  ...
  def _step(self, action):
    ...
    if self._done:
      if self._morl:
        return ts.termination_morl(observation, reward)
      else:
        return ts.termination(observation, reward)
    else:
      if self._morl:
        return ts.transition_morl(observation, reward, self._discount)  
      else:
