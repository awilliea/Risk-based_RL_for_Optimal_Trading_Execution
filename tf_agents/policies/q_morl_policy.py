...
class QPolicy(tf_policy.Base):
  ...
  def _distribution(self, time_step, policy_state):
    ...
    reward_size = time_step.observation[1].shape[-1]
    ...
    #print("Distribution q values shape:",q_values.shape)
    batch_size = q_values.shape[0]
    preference = tf.reshape(time_step.observation[1],(batch_size,reward_size,-1))
    q_values = tf.reshape(q_values, (batch_size, -1, reward_size))
    #print("Distribution q values shape:",q_values.shape)
    ...
    # for MORL, the shape of q_values is (batches, action dimensions, preference dimensions), it is needed to be transformed to (batches, utility dimenisons)
    
    if q_values.shape.rank == 3:
        q_values = tf.reshape(q_values, (batch_size, -1, reward_size))
        # TODO: need to down shape the preference(remove the repeated ones)
        logits = tf.reshape(tf.tensordot(q_values, preference, axes=((2),(1))), (batch_size, -1))
