...
def  element_wise_morl_squared_loss(labels, predictions, scope=None):
  ...
  with ops.name_scope(scope, "mean_squared_error",
    ...
    losses = tf.norm(losses, ord=2, axis=1)
    losses = math_ops.squared_difference(losses, tf.zeros(losses.shape, losses.dtype))
    return tf.reshape(losses,(losses.shape[0],-1))

def  element_wise_morl_l1_loss_with_preference(labels, predictions, scope=None):
...
def element_morl_huber_loss(labels, predictions, delta=1.0, scope=None):
  if labels is None:
    raise ValueError("labels must not be None.")
  if predictions is None:
    raise ValueError("predictions must not be None.")
  with ops.name_scope(scope, "huber_loss",
    ...
    error = tf.norm(error, ord=2, axis=1)
...
def element_morl_huber_loss_with_preference(labels, predictions, delta=1.0, scope=None):
  if labels is None:
    raise ValueError("labels must not be None.")
  if predictions is None:
    raise ValueError("predictions must not be None.")
