...
class TFAgent(tf.Module):
  ...
  def train(self, experience, weights=None, preference_batch=1):
    """Trains the agent.
    ...
    if self._enable_functions:
      if preference_batch == 1:
        ...
        loss_info = self._train_fn(experience=experience, weights=weights, preference_batch=preference_batch)
    else:
      if preference_batch == 1:
        loss_info = self._train(experience=experience, weights=weights)
      else:
        loss_info = self._train(experience=experience, weights=weights, preference_batch=preference_batch)
