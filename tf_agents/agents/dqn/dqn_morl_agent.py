...
from tf_agents.policies import q_policy, q_morl_policy
...
def compute_td_targets(next_q_values, rewards, discounts):
  #print("TD rewards",rewards.shape)
  #print("TD next_q_values",next_q_values.shape)
  #input("Wait")
  discounts = tf.reshape(discounts,(-1,1))
...
class DqnAgent(tf_agent.TFAgent):
  ...
  def __init__(
    ...
    self.beta = beta
    self.lamda = lamda
    self.lamda_init = lamda
    self.lamda_uplim = lamda_uplim
    self.lamda_expbase = lamda_expbase
    self.lamda_delta = lamda_delta
    self.homotopy = homotopy
    
    self._check_action_spec(action_spec)
    self._reward_size = time_step_spec.reward.shape[0]
    ...
    self._target_q_network = common.maybe_copy_target_network_with_checks(
        q_network, target_q_network, 'TargetQNetwork')

    self._epsilon_greedy = epsilon_greedy
    self._n_step_update = n_step_update
    self._boltzmann_temperature = boltzmann_temperature
    self._optimizer = optimizer
    self._td_errors_loss_fn1 = (
        td_errors_loss_fn[0] or common.element_morl_huber_loss)
    self._td_errors_loss_fn2 = (
        td_errors_loss_fn[1] or common.element_morl_huber_loss_with_preference)
    self._td_errors_loss_fn = [self._td_errors_loss_fn1, self._td_errors_loss_fn2]
    self._gamma = gamma
  ...
  def _update_lamda(self):
    if self.homotopy:
      self.lamda += self.lamda_delta
      self.lamda_delta = (self.lamda-self.lamda_init)*self.lamda_expbase+self.lamda_init-self.lamda
      
  def _setup_policy(self, time_step_spec, action_spec,
                    boltzmann_temperature, emit_log_probability):

    policy = q_morl_policy.QPolicy(
        time_step_spec,
        action_spec,
    ...
    target_policy = q_morl_policy.QPolicy(
        time_step_spec,
        action_spec,
  ...
  def _train(self, experience, weights, preference_batch=1):
  ...
  def generate_w(self, num_prefence, reward_size, fixed_w=None):
    frontier = np.array([[1.0,0.0],[0.8,0.2],[0.6,0.4],[0.4,0.6],[0.2,0.8],[0.0,1.0]])
    if fixed_w is not None:
        batch_size = fixed_w.shape[0]
        w = np.random.randn(num_prefence-7, reward_size)
        # normalize as a simplex
        w = np.abs(w) / np.linalg.norm(w, ord=1, axis=1).reshape(num_prefence-7, 1)
        w = np.concatenate((frontier,w),axis=0)
        w = np.repeat(w,batch_size,axis=0).reshape(batch_size*(num_prefence-1),reward_size)
        w = tf.constant(w, dtype=tf.float32)
        fixed_w = tf.reshape(fixed_w, (batch_size,reward_size))
        return tf.concat([fixed_w, w],axis=0)
  ...
  def _loss(self,
    ...
    with tf.name_scope('loss'):
      preference = time_steps.observation[1]
      batch_size = preference.shape[0]
      if preference_batch!=1:
        features =  time_steps.observation[0]
        step_type = time_steps.step_type
        next_features = next_time_steps.observation[0]
        reward = next_time_steps.reward
        discount = next_time_steps.discount
        next_step_type = next_time_steps.step_type
        
        reward_size = preference.shape[-1]
        preference = self.generate_w(preference_batch, reward_size, fixed_w=preference) #(batch*w_batch, reward_size)
        batch_size = preference.shape[0]
        #print("Preference shape",preference.shape)
        
        # extend factors to match the dimension of preference
        features = tf.repeat(features, preference_batch, axis=0)
        next_features = tf.repeat(next_features, preference_batch, axis=0)
        actions = tf.repeat(actions, preference_batch, axis=0)
        reward = tf.repeat(reward, preference_batch, axis=0)
        discount = tf.repeat(discount, preference_batch, axis=0)
        step_type = tf.repeat(step_type, preference_batch, axis=0)
        next_step_type = tf.repeat(next_step_type, preference_batch, axis=0)
        #print("features shape",features.shape)
        time_steps = time_steps._replace(observation=(features, preference), step_type=step_type)
        next_time_steps = time_steps._replace(observation=(next_features, preference), \
                                              reward=reward, discount=discount, step_type=next_step_type)
      ...
      #print("Loss caculation q_values shape:",q_values.shape)
      #print("Loss caculation next_q_values shape:",next_q_values.shape)
      if self._n_step_update == 1:
        ...
        #print("Loss caculation td_targets shape:",td_targets.shape)
        #input("Wait")
      else:
      ...
      valid_mask = tf.reshape(tf.cast(~time_steps.is_last(), tf.float32), (-1,1))
      td_error = valid_mask * (td_targets - q_values)
      td_error = tf.where(td_error <= 0, tf.math.multiply(td_error,1+self.beta), tf.math.multiply(td_error,1-self.beta)) 
      wtd_targets = tf.reshape(tf.reduce_sum(tf.multiply(td_targets, preference), axis=1), (batch_size,-1))
      wq_values = tf.reshape(tf.reduce_sum(tf.multiply(q_values, preference), axis=1), (batch_size,-1))
      #print("wtd_targets",wtd_targets.shape)
      #print("wq_values",wq_values.shape)
    
      td_loss1 = valid_mask * td_errors_loss_fn[0](td_targets, q_values)
      td_loss2 = valid_mask * td_errors_loss_fn[1](wtd_targets, wq_values)
      #print("td_loss1",td_loss1.shape)
      #print("td_loss2",td_loss2.shape)  
      #input("Wait")  
      td_loss = tf.math.add(tf.math.multiply((1-self.lamda),td_loss1) , tf.math.multiply(self.lamda,td_loss2))
  ...
  def _compute_q_values(self, time_steps, actions, training=False):
    ...
    q_values = tf.reshape(q_values, (q_values.shape[0], -1, self._reward_size))
    #print("Reshape Q values shape",q_values.shape)
  ...
  def _compute_next_q_values(self, next_time_steps):
    ...
    #print("Next_target_value shape:",next_target_q_values.shape)
    #print("batch size",batch_size)
    dummy_state = self._target_greedy_policy.get_initial_state(batch_size)
    #print("dummy state",dummy_state)
    next_target_q_values = tf.reshape(next_target_q_values, (next_target_q_values.shape[0],-1, self._reward_size))
    #input("Wait")
...
class DdqnAgent(DqnAgent):
  ...
  def _compute_next_q_values(self, next_time_steps):
    ...
    next_target_q_values, _ = self._target_q_network(
        network_observation, next_time_steps.step_type)
    #print("Nest_target_value shape:",next_target_q_values.shape)
